...
def save_many(query, comment=''):
    return request(
        '/test/save_many',
        method='POST',
        data=urlencode({'query': simplejson.dumps(query), 'comment': comment}),
    )
...
class DocumentTest(InfobaseTestCase):
    ...
    def test_versions(self):
        ...
        d = request(
            '/test/versions',
            method='GET',
            data={'query': simplejson.dumps({'key': '/new_page'})},
        )
        ...
        d = request(
            '/test/versions',
            method='GET',
            data={'query': simplejson.dumps({'limit': 1})},
        )
        self.assertEquals2(d, [{'key': '/new_page', 'revision': 1, '*': True}])
    ...
    def test_save_many(self):
        ...
        d = request(
            '/test/save_many',
            method='POST',
            data=urlencode({'query': simplejson.dumps(q)}),
        )
        self.assertEqual(
